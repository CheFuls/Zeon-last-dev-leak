...
public class CustomCrystalAuraV1 extends Module {
    ...
    private final Setting<BreakMode1> BreakMode = sgBreak.add(new EnumSetting.Builder<BreakMode1>()
            .name("break-mode")
            .description("The mode for a breaking crystals.")
            .defaultValue(BreakMode1.ignor_immunity)
            .build()
    ...
    private final Setting<Object2BooleanMap<EntityType<?>>> entities = sgTarget.add(new EntityTypeListSetting.Builder().name("entities").description("Select entities to draw nametags on.").defaultValue(Utils.asO2BMap(EntityType.PLAYER)).build());
    ...
    private final Setting<Boolean> antifriend = sgExtra.add(new BoolSetting.Builder()
            .name("anti-friend-pop")
            .description("Dont break crystal, when you can pop friend.")
            .defaultValue(false)
            .build()
    );
    
    private final Setting<Boolean> antiown = sgExtra.add(new BoolSetting.Builder()
            .name("anti-own-pop")
            .description("Dont break crystal, when you can pop you.")
            .defaultValue(false)
            .build()
    );
    ...
    public CustomCrystalAuraV1() {
        super(Zeon.Combat, "crystal-aura-V1", "Normal version.");
    }
    ...
    private void singleBreak(){
        ...
        getCrystalStream().max(Comparator.comparingDouble(o -> DamageUtils.crystalDamage(target1, o.getPos())))
    ...
    private void doHeldCrystal(){
        ...
        if (switchMode.get() != SwitchMode.None) doSwitch();
        if (mc.player.getMainHandStack().getItem() != Items.END_CRYSTAL && mc.player.getOffHandStack().getItem() != Items.END_CRYSTAL) return;
        bestDamage = DamageUtils.crystalDamage(target1, bestBlock.add(0, 1, 0));
    ...
    private void placeBlock(Vec3d block, Hand hand){
        ...
        if (render.get()) {
            ...
            renderBlock.damage = DamageUtils.crystalDamage(target1, bestBlock.add(0.5, 1, 0.5));
                    ...
                    if(isValid(new BlockPos(pos)) && getDamagePlace(new BlockPos(pos).up())){
                        if (!oldPlace.get() || isEmpty(new BlockPos(pos.add(0, 2, 0)))) {
                            if (!rayTrace.get() || pos.distanceTo(new Vec3d(mc.player.getX(), mc.player.getY() + mc.player.getEyeHeight(mc.player.getPose()), mc.player.getZ())) <= placeWallsRange.get() || rayTraceCheck(new BlockPos(pos), false) != null) {
                                if (!multiTarget.get()) {
                                    if (isEmpty(new BlockPos(pos)) && bestSupportDamage < DamageUtils.crystalDamage(target1, pos.add(0.5, 1, 0.5))) {
                                        bestSupportBlock = pos;
                                        bestSupportDamage = DamageUtils.crystalDamage(target1, pos.add(0.5, 1, 0.5));
                                    } else if (!isEmpty(new BlockPos(pos)) && bestDamage < DamageUtils.crystalDamage(target1, pos.add(0.5, 1, 0.5))) {
                                        bestBlock = pos;
                                        bestDamage = DamageUtils.crystalDamage(target1, bestBlock.add(0.5, 1, 0.5));
                                    }
                                } else {
                                    for (Entity entity : mc.world.getEntities()) {
                                        if (entity != mc.player && entities.get().getBoolean(entity.getType()) && mc.player.distanceTo(entity) <= targetRange.get()
                                                && entity.isAlive() && entity instanceof PlayerEntity
...
@EventHandler
	...
	if(render.get()) {
	event.renderer.box(x, y, z, x1, y1, z1, sideColor.get(), lineColor.get(), shapeMode.get(), 0);
        }
}
