...
public class CustomCrystalAuraV4 extends Module {
    ...
    public CustomCrystalAuraV4() {
        super(Zeon.Combat, "crystal-aura-v3", "Automatically places and attacks crystals.");
    }
    ...
    private void onPreTick(TickEvent.Pre event) {
        for(Entity entity : mc.world.getEntities()) {
        	if(entity instanceof EndCrystalEntity && fastBreak.get()) {
        		if(BlockUtilsWorld.distanceBetween(mc.player.getBlockPos(), entity.getBlockPos()) <= breakRange.get()) {
    ...
    public void onSpawnCrystal(EntitySpawnEvent e) {
        if (e.entity instanceof EndCrystalEntity && fastBreak.get() && BlockUtilsWorld.distanceBetween(mc.player.getBlockPos(), e.entity.getBlockPos()) < breakRange.get()) {

            if (rotate.get() && (yawStepMode.get() == YawStepMode.Break || yawStepMode.get() == YawStepMode.All))
                BlockUtilsWorld.rotateEnt(e.entity);
...
//        if (bestTarget instanceof PlayerEntity) return ((PlayerEntity) bestTarget);
//        else return null;
//    }
