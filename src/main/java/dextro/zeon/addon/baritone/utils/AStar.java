		...
		outer: for (BlockPos pos : positions2) {
			if (!isSolid(pos) && !closed.contains(pos)) {
				...
				for (BlockPos check : checkPositions2) {
					if (lava && !EntityUtils.isInRenderDistance(check)) {
						return null;
					}
					
					if (isSolid(check) || !EntityUtils.isInRenderDistance(check)) {
						continue outer;
					}
					
					if (BlockUtilsWorld.getBlock(check) == Blocks.LAVA && lava) {
						continue outer;
					}
					
					if (250 != -1 && check.getY() > 250) {
						continue outer;
					}
				}
